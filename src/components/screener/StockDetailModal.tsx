...
export function StockDetailModal({ ticker, isOpen, onClose, activeStrategy }: StockDetailModalProps) {
  ...
  const getStrategyExplanation = () => {
    ...
    switch (activeStrategy) {
      case 'scalping':
        return `This stock matches the Scalping Strategy due to high volatility (Beta: ${stock.beta}) and elevated trading volume (${formatMarketCap(stock.volume)} shares traded). Price has broken through key levels with a significant intraday move of ${formatPercent(stock.changePercent)}.`;
      case 'swing':
        return `This stock matches the Swing Pullback Strategy. It exhibits a strong structural uptrend with SMA(50) $${stock.sma50} trading above SMA(200) $${stock.sma200}. The RSI(14) of ${stock.rsi14} indicates it is currently in a healthy pullback / accumulation zone (between 40 and 60), presenting a high-probability swing trade entry.`;
      case 'momentum':
        return `This stock matches the Momentum Breakout Strategy. It is exhibiting massive relative strength, trading comfortably above its 20, 50, and 200-day simple moving averages with a highly bullish RSI(14) of ${stock.rsi14}. Crucially, its current price of $${stock.price} is within 5% of its 52-week high ($${stock.high52Week}), signaling a potential explosive breakout.`;
      case 'fundamental':
        return `This stock matches the Fundamental Value/Growth Strategy. It represents a highly profitable, financially stable large-cap company (Market Cap: $${(stock.marketCap / 1e9).toFixed(1)}B) trading at a highly reasonable valuation (P/E: ${stock.pe}x) with positive earnings per share (EPS: $${stock.eps}). The fact that price is above SMA(200) confirms structural long-term bullish support.`;
      default:
        return recommendation?.summary || `This stock shows clean indicators. Price is at $${stock.price} with an RSI(14) of ${stock.rsi14} and annual volatility ATR of $${stock.atr}. It represents a solid candidate for further monitoring.`;
    }
  };
      ...
      <DialogContent className="max-w-4xl bg-zinc-950 border-zinc-800 text-zinc-100 max-h-[90vh] overflow-hidden flex flex-col p-0">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-96 gap-3">
            ...
            <p className="text-sm text-zinc-400 font-mono">Retrieving quantitative metrics...</p>
          </div>
        ) : error ? (
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-zinc-900/60 border border-zinc-900 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[10px] text-zinc-500 font-mono">Daily Candles Charting</span>
                  </div>
                ...
                <Tabs defaultValue="technicals" className="w-full">
                  ...
                  <TabsContent value="technicals" className="pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-zinc-900/40 border border-zinc-900 rounded-xl p-4 space-y-3.5">
                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Moving Averages</span>
                    ...
                    <div className="bg-zinc-900/40 border border-zinc-900 rounded-xl p-4 space-y-3.5">
                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Momentum & Volatility</span>
                      ...
                      <div className="space-y-2 text-xs font-mono">
                        <div className="flex justify-between items-center">
                          ...
                          <span className={stock.rsi14 > 70 ? 'text-amber-500 font-bold' : stock.rsi14 < 30 ? 'text-purple-500 font-bold' : 'text-zinc-300'}>
                            {stock.rsi14.toFixed(1)} ({stock.rsi14 > 70 ? 'Overbought' : stock.rsi14 < 30 ? 'Oversold' : 'Neutral'})
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-zinc-400">MACD Line</span>
                    ...
                    <div className="col-span-1 md:col-span-2 bg-zinc-900/40 border border-zinc-900 rounded-xl p-4 flex flex-col md:flex-row justify-between gap-4 font-mono text-xs text-zinc-400">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-zinc-200">{formatMarketCap(stock.avgVolume3M)} shares</span>
                      </div>
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="fundamentals" className="pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-zinc-900/40 border border-zinc-900 rounded-xl p-4 space-y-3.5">
                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Valuation Ratios</span>
                      
                      <div className="space-y-2 text-xs font-mono">
                        <div className="flex justify-between items-center">
                          <span className="text-zinc-400">Trailing P/E</span>
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-zinc-400">Earnings Per Share (EPS)</span>
                    ...
                    <div className="bg-zinc-900/40 border border-zinc-900 rounded-xl p-4 space-y-3.5">
                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Shareholder Metrics</span>
                      
                      <div className="space-y-2 text-xs font-mono">
                        <div className="flex justify-between items-center">
                          <span className="text-zinc-400">Market Capitalization</span>
                          <span className="font-semibold text-zinc-200">
                            {formatMarketCap(stock.marketCap)}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-zinc-400">Beta (Volatility)</span>
                          <span className="font-semibold text-zinc-200">{stock.beta.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-zinc-400">Average Volume 3M</span>
                          <span className="font-semibold text-zinc-200">
                  ...
                  <TabsContent value="news" className="pt-4 space-y-3">
                    {news.length > 0 ? (
                      ...
                      <div className="text-center py-8 text-zinc-500 text-xs font-mono bg-zinc-900/20 border border-zinc-900 rounded-xl">
                        No news articles found for {stock.ticker}
                      </div>
                    )}
                  </TabsContent>
                </Tabs>
